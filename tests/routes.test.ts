...
const request = { start: "oasis", end: "hisaya", via: ["osu"], placeId: null };
const choice = {
  stops: [{ placeId: "osu", reason: "門前町が好きという会話から" }],
  explanation: "大須を通る散歩",
};
function repo() {
...
test("AI stops persist without a key; retry reuses AI selection and orders all waypoints", async () => {
  ...
  expect((await service.create("A", request)).status).toBe("needs-key");
  expect(r.readRoute("B")).toBeUndefined();
  expect(routed).toHaveLength(0);
  configured = true;
  expect((await service.create("A", request)).status).toBe("ready");
  expect(calls).toBe(1);
  expect(routed[0]).toEqual(["oasis", "osu", "hisaya"]);
  await service.create("A", request);
  expect(routed).toHaveLength(1);
  expect(r.read("A").points).toHaveLength(0);
});
test("missing mandatory stops, fabricated IDs and stale GPS are rejected", async () => {
  for (const stops of [[], [{ placeId: "invented", reason: "x" }]]) {
    ...
    await expect(s.create("A", { ...request, start: "gps" })).rejects.toThrow(
      /GPS/,
    );
  }
});
test("ORS receives walking coordinates, validates snapped waypoints and never substitutes a straight line on failure", async () => {
      ...
      features: [
        {
          ...
          properties: {
            summary: { distance: 1000, duration: 700 },
            way_points: [0, 1, 2],
          },
        },
      ],
  ...
  const result = await new OpenRouteService("test-key", fetcher).route(points);
  expect(result.distance).toBe(1000);
  expect(sent.url).toContain("foot-walking/geojson");
  expect(sent.body.coordinates).toEqual(points.map((p) => [p.lng, p.lat]));
  await expect(
    new OpenRouteService(
      "test",
      async () => new Response("", { status: 429 }),
    ).route(points),
  ).rejects.toThrow(/利用上限/);
  ...
  await expect(new OpenRouteService("test", bad).route(points)).rejects.toThrow(
    /指定地点/,
  );
  const r = repo(),
    s = new RouteService(
      r,
      { planRoute: async () => choice },
      {
        configured: () => true,
        route: async () => {
          throw Error("接続失敗");
  ...
  const failed = await s.create("A", request);
