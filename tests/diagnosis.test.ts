...
const track = (origin: TrackPoint["origin"] = "device") =>
  Array.from({ length: 7 }, (_, i) => ({
    id: `${origin}-${i}`,
    lng: places[0].lng,
    lat: places[0].lat,
    time: new Date(now - (10 - i) * 60000).toISOString(),
    accuracy: 5,
    origin,
  }));
test("stay needs sustained recent real observations; gaps, unknown accuracy and synthetic data do not prove a stay", () => {
  ...
  expect(
    diagnosisInput({ ...state, points: track() }, places, now).evidence,
  ).toHaveLength(1);
  for (const points of [
    track("synthetic"),
    track().map((p) => ({ ...p, accuracy: null })),
    track().slice(0, 4),
    track().filter((_, i) => i === 0 || i === 6),
  ])
    expect(
      diagnosisInput({ ...state, points }, places, now).evidence,
    ).toHaveLength(0);
  expect(
    diagnosisInput({ ...state, points: track() }, places, now + 31 * 86400000)
      .evidence,
  ).toHaveLength(0);
});
test("unknown remains null; fabricated evidence and duplicated axes are rejected", () => {
  const input = {
    evidence: [
      { id: "message:1", kind: "conversation" as const, text: "自然が好き" },
    ],
  };
  ...
  expect(() =>
    validateDiagnosis(
      { ...result, axes: result.axes.map((a) => ({ ...a, id: "nature" })) },
      input,
    ),
  ).toThrow();
...
test("diagnosis cache is per actor and changes with evidence; empty state never calls AI", async () => {
  ...
  await Promise.all([service.diagnose("A"), service.diagnose("A")]);
  expect(calls).toBe(1);
  await service.diagnose("A");
  expect(calls).toBe(1);
