...
export function walkerPosition(points: TrackPoint[]) {
  const point = points.reduce<TrackPoint | undefined>(
    (last, p) => (!last || p.time >= last.time ? p : last),
    undefined,
  );
  return {
    lng: point?.lng ?? 136.90855,
    lat: point?.lat ?? 35.16685,
    label: !point
      ? "デモのスタート地点"
      : point.origin === "device"
        ? "最後に取得したGPS位置"
        : point.origin === "synthetic"
          ? "散歩の再生位置"
          : "読み込んだ記録の位置",
  };
}
...
export function walkerCircle(lng: number, lat: number): Feature<Polygon> {
  const coordinates = Array.from({ length: 65 }, (_, i) => {
    ...
    return [
      lng + (20 * Math.cos(angle)) / (111320 * Math.cos((lat * Math.PI) / 180)),
